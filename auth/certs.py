...
CLIENT_CERTS[
    "kube-proxy/apiserver"] = {'cn': 'kube-proxy', 'cert_type': 'client'}
...
SERVER_CERTS["etcd/client"] = {'cn': 'etcd',
                               'cert_type': 'server', 'sans': getenv('trimmed_etcd_sans')}
# The server cert that etcd presents to peers.
SERVER_CERTS["etcd/peer"] = {'cn': 'etcd',
                             'cert_type': 'server', 'sans': getenv('trimmed_etcd_sans')}
# The server cert kube-apiserver presents to clients.
SERVER_CERTS["apiserver"] = {'cn': 'apiserver', 'cert_type': 'server', 'sans': getenv('trimmed_apiserver_sans'),
                             'needs_svcacctkey': True}
# The server cert kubelet presents to kube-apiserver (not used currently).
SERVER_CERTS["kubelet/server"] = {'cn': 'kubelet',
                                  'cert_type': 'server', 'sans': getenv('trimmed_kubelet_sans')}
# The server cert the authn webhook presents to kube-apiserver.
SERVER_CERTS["authn_webhook"] = {
    'cn': 'admin', 'cert_type': 'server', 'sans': getenv('trimmed_auth_webhook')}
# The server cert the kubernetes dashboard presents to users.
SERVER_CERTS["dashboard"] = {
    'cn': 'dashboard ', 'cert_type': 'server', 'sans': getenv('trimmed_dashboard_sans')}
