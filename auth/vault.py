...
def mock_root_ca(client):
    ...
    ca_response = client.secrets.pki.generate_root(
        type='internal', common_name='ca.com')
...
def get_client():
    ...
    token = subprocess.check_output(
        'vault token create -format=json | jq -r .auth.client_token', shell=True)
    if not token:
...
def generate_and_sign_intermediate(client, cluster_name):
    ...
    signed_intermediate = mock_sign_intermediate(
        client, csr, cluster_name, cn).json()['data']['certificate']
    print()
    print('SIGNED INTERMEDIATE')
    print(signed_intermediate)
    client.secrets.pki.set_signed_intermediate(
        signed_intermediate, mount_point=cluster_name)
